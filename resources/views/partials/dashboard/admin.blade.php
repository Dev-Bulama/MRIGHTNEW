...
<div class="row mb-4">
    ...
    $recentPendingShops = \App\Models\Shop::where('approved', false)
        ->with('user')
        ->whereHas('user') // Only shops with existing users
        ->latest()
        ->take(5)
        ->get();
@endphp
...
<div class="row">
    ...
    @php
        ...
        // Recent shop registrations - with null safety
        $recentShops = \App\Models\Shop::with('user')->latest()->take(3)->get();
        foreach($recentShops as $shop) {
            $userName = $shop->user ? ($shop->user->first_name ?? $shop->user->name ?? 'Unknown User') : 'Deleted User';
            
            $recentActivities[] = [
                'icon' => 'fas fa-store',
                'color' => 'primary',
                'title' => 'New Shop Registration',
                'description' => $shop->shop_name . ' by ' . $userName,
                'time' => $shop->created_at,
                'status' => $shop->approved ? 'approved' : 'pending'
            ];
        }
        
        // Recent receipts - with null safety
        $recentReceipts = \App\Models\Receipt::with(['shop', 'user'])->latest()->take(3)->get();
        foreach($recentReceipts as $receipt) {
            $shopName = $receipt->shop ? $receipt->shop->shop_name : 'Unknown Shop';
            
            $recentActivities[] = [
                'icon' => 'fas fa-receipt',
                'color' => 'success',
                'title' => 'Receipt Generated',
                'description' => $receipt->receipt_number . ' by ' . $shopName,
