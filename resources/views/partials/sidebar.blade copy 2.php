   ...
   <div class="user-profile p-4 border-bottom bg-light">
    <div class="d-flex align-items-center">
        <div class="avatar me-3">
            ...
            <img src="{{ auth()->check() && auth()->user()->avatar_url ? auth()->user()->avatar_url : 'https://via.placeholder.com/40x40/667eea/ffffff?text=U' }}"
                 alt="{{ auth()->user()->name }}" 
                ...
                @else
                    ...
                    <a class="nav-link {{ request()->routeIs('shop.payouts.*') ? 'active' : '' }}" 
                       href="{{ route('shop.payouts.index') }}">
                        ...
                        @php 
                            $pendingPayouts = auth()->user()->pendingPayoutRequests()->count();
                            $availableBalance = auth()->user()->getAvailableCommissionBalance();
                        @endphp
                        @if($pendingPayouts > 0)
                            <span class="badge bg-warning ms-auto">{{ $pendingPayouts }}</span>
                        @elseif($availableBalance >= \App\Models\PayoutRequest::getMinimumPayoutAmount())
