...
<body>
        ...
        <div class="col-50">
            ...
            <div class="info-section">
                ...
                @elseif($receipt->notes)
                    {{-- Agent receipt: shop details are in notes --}}
                    <p style="white-space:pre-line;">{{ $receipt->notes }}</p>
                    <p><strong>Generated by:</strong><br>{{ $receipt->user->name ?? 'Agent' }} (Agent)</p>
                @else
    ...
    @php
        ...
        $pin   = $receipt->resale_pin;
        // Show first 4 digits of phone, mask middle, show last 2
        $maskedPhone = strlen($phone) > 6
            ? substr($phone, 0, 4) . str_repeat('*', strlen($phone) - 6) . substr($phone, -2)
            : $phone;
        // Show first 2 digits of PIN, mask rest
        $maskedPin = substr($pin, 0, 2) . '****';
    @endphp
    <div style="background:#FFF8F8;border:2px solid #C0392B;border-radius:6px;padding:12px;margin-bottom:10px;font-size:10px;">
        <strong style="color:#C0392B;font-size:11px;">🔐 YOUR RECEIPT PORTAL LOGIN</strong><br>
        <span style="color:#555;">Keep this receipt safe. Use the details below to access your digital receipt portal:</span><br><br>
        <table style="width:100%;border-collapse:collapse;">
            <tr>
                <td style="padding:3px 8px;background:#C0392B;color:white;font-weight:bold;width:45%;border-radius:3px 0 0 3px;">Phone Number</td>
                <td style="padding:3px 8px;background:#F8F8F8;border:1px solid #ddd;font-family:monospace;font-weight:bold;">{{ $maskedPhone }}</td>
            </tr>
            <tr>
                <td style="padding:3px 8px;background:#C0392B;color:white;font-weight:bold;border-radius:0 0 0 3px;">Resale PIN</td>
                <td style="padding:3px 8px;background:#F8F8F8;border:1px solid #ddd;font-family:monospace;font-weight:bold;">{{ $maskedPin }}</td>
            </tr>
        </table>
        <span style="color:#888;font-size:9px;">⚠ Only partial digits shown for security. Remember your full phone number and PIN.</span><br>
        <span style="color:#7F8C8D;">Portal URL: <strong>{{ url('/receipt-portal/login') }}</strong></span>
    </div>
    @endif
