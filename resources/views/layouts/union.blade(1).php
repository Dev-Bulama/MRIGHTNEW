...
<!-- Navigation -->
            ...
            <div class="sidebar-nav">
                @php
                    ...
                    // ✅ CORRECT PERMISSION-BASED NAVIGATION using underscore notation from your Role model
                    ...
                    $hasReceiptAccess = $user->hasAnyPermission(['view_receipts', 'manage_receipts']);
                    $hasPayoutAccess = $user->hasAnyPermission(['view_payouts', 'approve_payouts', 'process_payments']);
                    
                    // Reports & Analytics Permissions
                    $hasReportAccess = $user->hasAnyPermission(['view_reports', 'export_data', 'view_analytics']);
                    
                    // User Management Permissions - REMOVED since route doesn't exist
                    // $hasUserAccess = $user->hasAnyPermission(['view_users', 'manage_users', 'edit_users']);
                @endphp

                <!-- Dashboard - Always visible to union users -->
                <div class="nav-item">
                    <a href="{{ route('union.dashboard') }}" class="nav-link {{ request()->routeIs('union.dashboard') ? 'active' : '' }}">
                        <i class="fas fa-tachometer-alt"></i>
                        <span class="nav-text">Dashboard</span>
                    </a>
                </div>

                <!-- Shop Management Section - Only show if user has shop permissions -->
                @if($hasShopAccess || $hasPreApprovalAccess)
                    <div class="nav-section-header">Shop Management</div>
                    
                    @if($hasShopAccess)
                        <div class="nav-item">
                            <a href="{{ route('union.shop-owners.index') }}" class="nav-link {{ request()->routeIs('union.shop-owners.*') ? 'active' : '' }}">
                    ...
                    @if($hasPreApprovalAccess)
                        <div class="nav-item">
                            <a href="{{ route('union.pre-approvals.index') }}" class="nav-link {{ request()->routeIs('union.pre-approvals.*') ? 'active' : '' }}">
                ...
                <!-- Financial Section - Only show if user has financial permissions -->
                @if($hasReceiptAccess || $hasPayoutAccess)
                    <div class="nav-section-header">Financial</div>
                    
                    @if($hasReceiptAccess)
                        <div class="nav-item">
                            <a href="{{ route('union.receipts.index') }}" class="nav-link {{ request()->routeIs('union.receipts.*') ? 'active' : '' }}">
                    ...
                    @if($hasPayoutAccess)
                        <div class="nav-item">
                            <a href="{{ route('union.payouts.index') }}" class="nav-link {{ request()->routeIs('union.payouts.*') ? 'active' : '' }}">
                                ...
                                @endphp
                                @if($pendingPayouts > 0)
                                    <span class="badge bg-warning ms-auto">{{ $pendingPayouts }}</span>
                                @endif
                            </a>
                        </div>
                    @endif
                ...
                <!-- Reports Section - Only show if user has reporting permissions -->
                @if($hasReportAccess)
                    <div class="nav-section-header">Reports & Analytics</div>
                    
                    <div class="nav-item">
                        <a href="{{ route('union.reports.index') }}" class="nav-link {{ request()->routeIs('union.reports.*') ? 'active' : '' }}">
