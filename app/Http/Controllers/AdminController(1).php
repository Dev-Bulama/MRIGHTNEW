...
public function userDetails(User $user)
{
    ...
    // public function userDetails(User $user)
    // {
    //     $user->load(['shop', 'receipts' => function($query) {
    //         $query->latest()->take(5);
    //     }]);
        
    //     $userStats = [
    //         'total_receipts' => $user->receipts()->count(),
    //         'total_spent' => $user->receipts()
    //             ->where('payment_gateway_status', 'successful')
    //             ->sum('amount') ?? 0,
    ...
    //     $html = view('admin.users.details-modal', compact('user', 'userStats'))->render();
     ...
     */
   /**
 * Delete user (PERMANENT WITH BACKUP).
 */
 /**
 * Delete user (SIMPLE PERMANENT DELETION).
 */
 /**
...
//         // 1. DELETE ASSOCIATED SHOP FIRST (if exists)
//         if ($user->shop) {
//             $user->shop->forceDelete();
//         }

//         // 2. DELETE ALL USER'S RECEIPTS (permanent)
//         $user->receipts()->forceDelete();

//         // 3. DELETE ALL OTHER RELATED DATA
//         $user->payments()->delete();
//         $user->payoutRequests()->delete();
...
//         // 1. CREATE BACKUP RECORD BEFORE DELETION
...
//                 'shop_data' => $user->shop ? $user->shop->toArray() : null,
//                 'receipts_count' => $user->receipts()->count(),
//                 'total_earnings' => $user->receipts()->sum('amount')
//             ]),
//             'deleted_at' => now(),
//             'deleted_by' => auth()->id()
//         ]);

//         // 2. FORCE DELETE ASSOCIATED SHOP (but keep receipts for accounting)
//         if ($user->shop) {
//             $shop = $user->shop;
            
//             // Update all receipts to remove shop reference but keep for accounting
//             \DB::table('receipts')
//                 ->where('user_id', $user->id)
//                 ->update([
//                     'shop_deleted' => true,
//                     'original_shop_name' => $shop->shop_name,
//                     'updated_at' => now()
//                 ]);
...
//         // 3. UPDATE RECEIPTS TO PRESERVE FOR ACCOUNTING
//         \DB::table('receipts')
//             ->where('user_id', $user->id)
//             ->update([
//                 'user_deleted' => true,
...
//                     'notes' => "Credentials available for reuse after deletion of {$userName}",
//                     'created_at' => now(),
//                     'updated_at' => now()
//                 ]
//             );
...
public function bulkDeleteUsers(Request $request)
{
    try {
        ...
        return response()->json([
            'success' => true,
            'affected_count' => $affectedCount,
            'message' => "Successfully deleted {$affectedCount} users and all their data permanently."
        ]);
