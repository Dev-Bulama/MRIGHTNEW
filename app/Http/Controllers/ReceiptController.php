...
public function store(Request $request)
{
    ...
    try {
        ...
        // }
// If payment method is Paystack, skip payment page and go directly to initialization
if ($validated['payment_method'] === 'paystack') {
    // Get customer email from the form data
    ...
    try {
        ...
        $response = $paymentController->initialize($request, $receipt);
        
        // If it's a JSON response with authorization_url, redirect there
        if ($response instanceof \Illuminate\Http\JsonResponse) {
            $responseData = $response->getData(true);
            if (isset($responseData['success']) && $responseData['success'] && isset($responseData['data']['authorization_url'])) {
                return redirect($responseData['data']['authorization_url']);
            } else {
                // Payment initialization failed, redirect to payment page with error
                return redirect()->route('payment.show', $receipt)
                    ->with('error', $responseData['message'] ?? 'Payment initialization failed');
            }
        }
        ...
        \Log::error('Direct payment initialization failed: ' . $e->getMessage());
...
public function processResaleWithPayment(Request $request)
{
    ...
    try {
        ...
        if ($validated['payment_method'] === 'paystack') {
            // Get customer email from the form data
            $customerEmail = $validated['new_customer_email'];
            $customerPhone = $validated['new_customer_phone'] ?? null;
            ...
            try {
                ...
                // Initialize payment directly
                $paymentController = new \App\Http\Controllers\PaymentController();
                $response = $paymentController->initialize($request, $resaleReceipt);
                ...
                \Log::error('Direct resale payment initialization failed: ' . $e->getMessage());
