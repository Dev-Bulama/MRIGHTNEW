...
// Auto-create shop profile for pre-approved shop owners
        if ($preApproved && $request->user_type === 'shop_owner') {
            $shop = \App\Models\Shop::create([
                'user_id' => $user->id,
                'shop_name' => $preApproved->shop_name ?? 'Shop - ' . $user->name,
                'owner_full_name' => $user->name,
                ...
                'business_email' => $user->email,
                'country' => 'Nigeria',
                'state' => $preApproved->state ?? 'Unknown',
                'local_government' => $preApproved->local_government ?? 'Unknown',
                'approved' => true,
                'approved_at' => now(),
                'approved_by' => 1, // System admin ID
                'status' => 'active',
                'additional_info' => [
                    'registration_source' => 'pre_approved',
                    'auto_approved' => true,
                    'pre_approval_used_at' => now()->toDateTimeString(),
                ],
            ]);
            
            // Update user with shop relationship
            $user->update(['shop_id' => $shop->id]);
            
            // Log successful auto-creation
            \Log::info("Auto-approved shop created for pre-approved user: {$user->email}, Shop ID: {$shop->id}");
        }
        // Mark pre-approval as used
        if ($preApproved) {
            $preApproved->markAsUsed($user->id);
