  ...
  public function index(Request $request)
{
    ...
    // Calculate union payout data (N300 per receipt)
    $payoutData = [
        'current_month_receipts' => $user->getUnionReceiptsForMonth(),
        'current_month_commission' => $user->getCurrentMonthUnionCommission(),
        'total_commission_earned' => $user->getTotalUnionCommission(),
        'commission_history' => $user->getUnionCommissionHistory(12), // Last 12 months
        'formatted_current_commission' => '₦' . number_format($user->getCurrentMonthUnionCommission(), 2),
        'formatted_total_commission' => '₦' . number_format($user->getTotalUnionCommission(), 2),
    ];
...
/**
 * Get commission history for AJAX requests
 */
public function getCommissionHistory(Request $request)
{
    ...
    $history = $user->getUnionCommissionHistory($months);
    ...
    return response()->json([
        ...
        'summary' => [
            ...
            'total_receipts' => array_sum(array_column($history, 'receipts_count')),
            'total_commission' => array_sum(array_column($history, 'commission_amount')),
            'average_monthly_receipts' => count($history) > 0 ? round(array_sum(array_column($history, 'receipts_count')) / count($history)) : 0,
            'formatted_total_commission' => '₦' . number_format(array_sum(array_column($history, 'commission_amount')), 2),
        ]
    ]);
}
