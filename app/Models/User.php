...
/**
 * Get total receipts generated in union's assigned locations for specific month
 */
public function getUnionReceiptsForMonth($year = null, $month = null): int
{
    if (!$this->isUnion()) {
        return 0;
    }
    ...
    return Receipt::whereHas('shop', function($query) {
            $query->whereIn('state', $this->assigned_states ?? []);
            
            // If specific LGAs are assigned, filter by them too
            if (!empty($this->assigned_lgas)) {
                $query->whereIn('local_government', $this->assigned_lgas);
            }
        })
...
/**
 * Get union commission for specific month (N300 per receipt)
 */
public function getUnionCommissionForMonth($year = null, $month = null): float
{
    $receiptCount = $this->getUnionReceiptsForMonth($year, $month);
    return $receiptCount * 300; // N300 per receipt
}

/**
 * Get union commission for current month
 */
public function getCurrentMonthUnionCommission(): float
{
    return $this->getUnionCommissionForMonth();
}

/**
 * Get union commission history for multiple months
 */
public function getUnionCommissionHistory($months = 6): array
{
    $history = [];
    ...
    for ($i = 0; $i < $months; $i++) {
        ...
        $receiptsCount = $this->getUnionReceiptsForMonth($year, $month);
        $commission = $this->getUnionCommissionForMonth($year, $month);
...
/**
 * Get total union commission earned to date
 */
public function getTotalUnionCommission(): float
{
    if (!$this->isUnion()) {
        return 0;
    }
    
    $totalReceipts = Receipt::whereHas('shop', function($query) {
            $query->whereIn('state', $this->assigned_states ?? []);
    ...
    return $totalReceipts * 300; // N300 per receipt
}
